"use client";
import { rgbStringToHex } from "@/utils/strings";
import { useColorScheme, vars } from "nativewind";

function colorConfig(config: Record<string, string>) {
  return {
    original: config,
    parsed: Object.keys(config).reduce(
      (acc: Record<string, string>, curr: string) => {
        const propertyName = curr
          .replace("--color-", "")
          .split("-")
          .map((word, i) =>
          ...
          .join("");
        acc[propertyName] = rgbStringToHex(config[curr]);
        return acc;
      },
      {}
    ),
...
export const lightConfig = colorConfig({
...
export const darkConfig = colorConfig({
...
export const config = {
  light: vars(lightConfig.original),
  dark: vars(darkConfig.original),
};

export function useColorConfig() {
  const { colorScheme } = useColorScheme();
  return colorScheme === "dark" ? darkConfig : lightConfig;
}